...
export const updateUser = async (req, res, next) => {
  if (req.user.id !== req.params.id) //Check id of user
    return next(errorHandle(401, "You can only update your own account"))
  try {
    if (req.body.password) //Valid user, we hash password
    {
...
export const getUserListings = async (req, res, next) => {
  if (req.user.id === req.params.id) {
    ...
    return next(errorHandle(401, "You can only get user listings your own account!"))
  }
